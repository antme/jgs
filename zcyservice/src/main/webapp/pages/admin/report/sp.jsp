...
<body>
      <div class="page_tip">说明： 不输入时间查询所有的服务商的分布，默认显示一个月内注册的服务商的分布</div>

	  <div class="p_height_div"></div>
      <span><label style="margin-left:20px">注册日期：</label></span>
      ...
      <span class="span_style"><button class="public_btn" onclick="querySpData();">&nbsp</button></span>
	  <div class="p_height_div"></div>
      <div class="panl-order-mouth" >
             <ul >
                <li id="first_li" class="two click_back" onclick="querySpData(0);">本月</li>
                <li class="first" onclick="querySpData(1);">上个月</li>             
                <li class="first" onclick="querySpData(2);">前三个月</li>                
             </ul>
      </div>
    <div id="chart"></div>
    ...
    function querySpData(type){
        var data = {
                "startDate" : $('#hstartDate').datebox('getValue'),
                "endDate" : $('#hendDate').datebox('getValue'),
                "dateType" : type
            }
    	 postAjaxRequest("/ecs/sys/report/location/sp.do", data, function(response) {
    		 ...
    		 $('#chart').highcharts({
    		        ...
    		        title: {
    		            text: '服务商分布比例，总数：' + response.total
    		        },
    		        tooltip: {
