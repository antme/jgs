...
public class EmailUtil {
	...
	public static void sendMail(String subject, List<Object> toList, String content, String file) {
        ...
        try {
            ...
            email.setAuthentication(CFGManager.getProperty("email"), CFGManager.getProperty("password"));
            email.setHostName(CFGManager.getProperty("smtp"));// 设置发送主机的服务器地址
            email.setFrom(CFGManager.getProperty("email"), "工程管理系统");// 发件人邮箱
            
            List<InternetAddress> address = new ArrayList<InternetAddress>();
            for (Object mail : toList) {
                if (!EcUtil.isEmpty(mail)) {
            ...
            if (!address.isEmpty()) {
                ...
                if (!EcUtil.isEmpty(file) && new File(file).exists()) {
